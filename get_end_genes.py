...
def _get_analysis(maxgen, npool):
    for generation in xrange(maxgen+1):
        end_results = []
...
with open(get_run_dir()+"statespace/current_status.csv",'r') as fi:
    ...
    for row in reader:
        if row[0] == "gen":
            maxgen = int(row[1].strip('\n'))
        elif row[0] == "npool":
            npool = int(row[1].strip('\n'))
print "maxgen: ", str(maxgen), "\tnpool: ", str(npool), "\n"
_get_analysis(maxgen, npool)
