...
from get_general import _get_gen_npool
...
def _mean_distances(gene_dist_dict, npool):
    lastgen,npool = _get_gen_npool()
    ...
    with open(read_path, 'r') as fi:
        ...
        for line in list_of_lines:
            ...
            if gen != curr_gen:
                ...
                curr_gen += 1
                dist_sum = 0
            dist_sum += float(fitness)*int(freq)
        mean_dist = dist_sum / npool
        mean_dist_dict.update({curr_gen:mean_dist})
    fi.close()

    write_path = get_run_dir() + "statespace/mean_distances.csv"
...
_mean_distances(gene_dist_dict,npool)
