...
########################
# This function returns the job names of jobs to be run by the 
# molSimplify job manager.
...
########################
# This function gives a GA_run_definition object that contains
# all of the parameters for the GA, which are stored in the .madconfig
# folder of the base run directory.
def get_current_GA():
...
########################
# mADs previous way to handle and restart jobs was a bit confusing, but used
# a job file, and an input file. The job file did not have the coordinates or 
# the scratch directory labeled. The input file takes the job file and customizes
# the coordinates and scratch directory, which is what this function does.
...
#############
# Helper function to determine if certain keywords are present
# within a given input file. Returns bool.
def check_txt_infile(target_inpath, target_txt):
    return target_txt in open(target_inpath).read()


#########################
# Adds the properties needed for the dynamic classifier to the input
# files that are generated by molSimplify (for terachem).
...
######################
# Adds the scratch directory line to a specific input file. Slightly
# redundant to get_infile_from_job, but meant to be a general function
# to customize scratch directories to input files.
...
########################
# Given a certain jobname (gene within the context of mAD), this function
# returns where the initial geometry is. Irrelevant if using the job manager.
...
########################
# This function takes in key job information, such as a job gene, and then
# uses all of the information that it has to generate a generic input file.
...
# or a comparison class, which is needed for any sort of evolutionary optimization
# in a closed loop cycle. This is also what is responsible for gathering the info
# that is later written to the filesystem as "results_post".
...
########################
# Reads the list titled live_jobs.csv, which contains the jobs and their
# corresponding job numbers. Returns a dictionary of those jobs.
def find_live_jobs():
    path_dictionary = setup_paths()
...
########################
# Helper function to find the index of a ligand given its name.
# Particularly helpful for assembling gene names.
...
########################
# Returns the permitted oxidation states for exploration in the GA. 
...
# spin states to a given metal and oxidation state.
def spin_dictionary():
    GA_run = get_current_GA()
...
########################
# This dictionary is used to determine the final charge of a transition metal complex after dissociating
# a given ligand. Assumes heterolytic dissociation.
...
########################
# This function does not use ligand_breakdown because it is old. It has the number of atoms
# in the ligand that would be dissociated, so that the new molecule can be written.
...
########################
# This is a core function to mAD. Given a query, it fetches the madconfig file and returns
# the value of that keyword. If the keyword does not exist, it returns False.
...
########################
# This function is a legacy function that checks which optimizer should be used.
...
########################
# Takes the unreadable gene, which is a set of integers, and converts it into
...
########################
# This function allows you to find a complex within a mAD run, if you are looking
# for the gene (unreadable name) of a complex for which you know the chemical name.
# Should be necessary for use in the presense of the database.
...
########################
# Convenience function to put name together given all of the relevant pieces.
...
########################
# This function turns a smiles string with a given denticity to a ligand class.
...
#######################
# Helper function to handle the conversion of SMILES ligands.
...
########################
# Function for mADs DFT management capabilities, renames a 20% HFX with 25% etc.
...
########################
# Returns the basename of a job, without any modifiers (.in, .xyz, etc).
# You will get back an unreadable name --> gen_0_slot_10_0_3_6_6_6_6_4_4_20_1
def stripName(job):
    # gets base job name
    base = os.path.basename(job)
    base = base.strip("\n")
...
#######################
# Convenience function to name empty site structures from a given oxo structure.
...
#######################
# Convenience function to name hydroxyl structures generated from an oxo structure.
...
#######################
# Helper function to convert int strings to decimal strings.
def to_decimal_string(inp):
    # nuisance function to convert
...
#######################
# The dictionary that decides which order the Hartree Fock 
# resampling should take place.
def HFXordering():
