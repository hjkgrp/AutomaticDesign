...
echo "Begining water calcualtion run"
...
sourcepath=$fullpath
inpath=$generalpath/water_infiles/$gennumpath/$namebase.in
outpath=$generalpath/water_outfiles/$gennumpath/$namebase.out
scrpath=$generalpath/scr/water/$gennumpath/$namebase
echo "scr will be copied to  $scrpath"
echo "paths set"

echo "water inpath is $inpath"
echo "Initializing local run, finding input files..."
mkdir -p scr
mkdir -p scr/water/$gennumpath/
spacer='_'
echo "begining"
...
echo "Launching water calc: $namebase"
$TeraChem/bin/terachem $inpath >  $outpath
mv scr/water/$gennumpath/$namebase $scrpath
echo "Complete water"
