...
def draw_pca_fig(Xr,mod_dat,live_db_inds,all_inds,hull):
    ...
    for simplex in hull.simplices:
        plt.plot(points[simplex, 0], Xr[simplex, 1], 'k-')

    o1 = plt.tricontourf(Xr[all_inds,0], Xr[all_inds,1],mod_dat.loc[all_inds,'fitness'],
                   5,alpha=0.25,cmap=cmstr,linestyles='dashed',linewidths=1)
    for c in o1.collections:
        c.set_edgecolor("face")

    o2 = plt.scatter(Xr[all_inds,0], Xr[all_inds,1], c=mod_dat.loc[all_inds,"fitness"],
            cmap=cmstr,s=100,
                alpha=1, lw=1,marker=".",label='previous')
    o3 = plt.scatter(Xr[live_db_inds,0], Xr[live_db_inds,1], c=mod_dat.loc[live_db_inds,"fitness"],
            cmap=cmstr,s=125,
                alpha=1.0, lw=3,marker="s",label='current')
    cbar = fig.colorbar(o1)
    ...
    frame1.axes.tick_params(direction='in')
    plt.show()
...
def draw_double_plot(Xr,mod_dat,live_db_inds,all_inds,hull,gens,mfs,divs):
    ...
    o1 = ax1.tricontourf(Xr[all_inds,0], Xr[all_inds,1],mod_dat.loc[all_inds,'fitness'],
                   5,alpha=0.25,cmap=cmstr,linestyles='dashed')
    for c in o1.collections:
        c.set_edgecolor("face")
